...
public class GeraTabelaTmp {
	...
	public static void main(String[] args) {
		    	...
		    	waba.applet.JavaBridge.setNonGUIApp( );
		    	 ...
		    	 Settings.dataPath = "F:\\SIAB_MOBILE3\\SIAB\\CreatePdbTemp";
		
		String CREATORID = "SIAB";
		LitebaseConnection driver = LitebaseConnection.getInstance(CREATORID,"aspdb");
		driver.execute("CREATE TABLE FICHATBACOMPTEMP (CD_FAMILIA long,CD_PACIENTE long, CD_MES int,CD_ANO int,DT_VISITA char(20)," +
...
/*		driver.executeUpdate(" INSERT INTO FICHATBACOMP (CD_PACIENTE, CD_MES,DT_VISITA,FL_MEDICACAO, FL_REAINDESE, DT_CONSULTA, FL_ESCARRO, NUM_EXAMINADOS, NUM_COMBCG)" +
		...
		driver.execute("CREATE TABLE FICHATBTEMP (CD_FAMILIA long,CD_PACIENTE long,CD_ANO int, NUM_COMUNICANTES int, NUM_COMUNICANTES5 int, DS_OBSERVACAO char(100),STATUS int)");
...
*/
		driver.execute("CREATE TABLE FICHACADFAMTEMP (CD_FAMILIA long,CD_ANO int, DS_ENDERECO char(60), NUM_ENDERECO int," +
		...
		driver.execute("CREATE TABLE PACFAMILIATEMP (CD_FAMILIA long, CD_PACIENTE long,CD_ANO int, NM_PACIENTE CHAR(30)," +
		...
		driver.execute("CREATE TABLE FICHAHANTEMP (CD_FAMILIA long,CD_PACIENTE long,CD_ANO int, NUM_COMUNICANTES int, DS_OBSERVACAO char(100),STATUS int)");		
		driver.execute("CREATE TABLE FICHAHANACOMPTEMP (CD_FAMILIA long,CD_PACIENTE long, CD_MES int,CD_ANO int,DT_VISITA char(20)," +
		...
		driver.execute("CREATE TABLE FICHAHATEMP (CD_FAMILIA long,CD_PACIENTE long ,CD_ANO int, FL_FUMANTE char(1), DS_OBSERVACAO char(100),STATUS int)");		
		driver.execute("CREATE TABLE FICHAHAACOMPTEMP (CD_FAMILIA long,CD_PACIENTE long, CD_MES int,CD_ANO int, DT_VISITA char(20)," +
		"FL_DIETA char(1), FL_MEDICACAO char(1),FL_EXERCICIO char(1), DS_PRESSAO char(10), DT_ULTCONSULTA char(10),STATUS int)");
		
		driver.execute("CREATE TABLE FICHADIATEMP (CD_FAMILIA long,CD_PACIENTE long,CD_ANO int, DS_OBSERVACAO char(100),STATUS int)");		
		driver.execute("CREATE TABLE FICHADIAACOMPTEMP (CD_FAMILIA long,CD_PACIENTE long, CD_MES int, CD_ANO int,DT_VISITA char(20)," +
		"FL_DIETA char(1),FL_EXERCICIO char(1), FL_INSULINA char(1),FL_HIPOGLICEMIANTE char(1), DT_ULTCONSULTA char(10),STATUS int)");
		
		driver.execute("CREATE TABLE FICHAGESTEMP (CD_FAMILIA long,CD_PACIENTE long,CD_ANO int, DT_ULTREGRA char(10),DT_PARTO char(10)," +
		...
		driver.execute("CREATE TABLE FICHAGESACOMPTEMP (CD_FAMILIA long,CD_PACIENTE long, CD_MES int,CD_ANO int,FL_ESTADONUTRI char(1), DT_CONSPRENATAL char(10), DT_VISITA char(10),STATUS int)");
		
		
		driver.execute("CREATE TABLE FICHAIDOSOTEMP (CD_FAMILIA long,CD_PACIENTE long,CD_ANO int, FL_FUMANTE char(10),DT_DTP char(10)," +
	    ...
	    driver.execute("CREATE TABLE FICHAIDOSOACOMPTEMP (CD_FAMILIA long,CD_PACIENTE long, CD_MES int,CD_ANO int,FL_ESTADONUTRI char(1), DT_VISITA char(10), DT_ACOMPPSF char(10),STATUS int)");
	    
	    
	    driver.execute("CREATE TABLE FICHACRIANCATEMP (CD_FAMILIA long,CD_PACIENTE long,CD_ANO int, CD_AVALIACAOALIMEN int,DS_TIPOALIMENTACAO char(50)," +
	    ...
	    driver.execute("CREATE TABLE FICHACRIANCAACOMPTEMP (CD_FAMILIA long,CD_PACIENTE long, CD_MES int,CD_ANO int,DT_QUADRONUTRI char(10), NUM_PESO float, NUM_ALTURA float,STATUS int)");
	    
	    driver.execute("CREATE TABLE AGENTETEMP (CD_AGENTE long,NM_AGENTE char(30), DS_LOGIN char(10), DS_SENHA char(10), CD_SEG char(2), CD_AREA char(3), CD_MICROA char(2))");
	    
	    driver.execute("CREATE TABLE VALIDATIONTEMP (SEQUENCIAL int)");
	    
	    driver.execute("CREATE TABLE OCUPACAOTEMP (CD_OCUPACAO int, DS_OCUPACAO char(60),STATUS int)");
